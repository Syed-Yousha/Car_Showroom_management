...
class CustomersScreenState extends State<CustomersScreen> {
  ...
  int get _withBalanceCount => _customers.where((c) => _getBalance(c) > 0).length;
  ...
  void _showAddTransactionDialog(Map<String, dynamic> c) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDialogState) {
        return ContentDialog(
          title: const Text("Add Transaction", style: TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700)),
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (txnType == 'Sell Car') ...[
                ...
                if (selectedCar != null)
                  Container(
                    ...
                    child: Row(children: [
                      const Icon(FluentIcons.info, size: 14, color: AppTheme.primary),
                      const SizedBox(width: 8),
                      Expanded(child: Text("$selectedCar will be charged to ${c['name']}'s account", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, color: AppTheme.primary))),
                    ]),
                  ),
                const SizedBox(height: 14),
              ...
              if (txnType == 'Payment') ...[
                ...
                Text("Payment Method", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, fontWeight: FontWeight.w600, color: AppTheme.textSecondary)),
                const SizedBox(height: 8),
                Row(children: [
                  _paymentChip('Cash', paymentType, (v) => setDialogState(() => paymentType = v)),
                  const SizedBox(width: 8),
                  _paymentChip('Bank Transfer', paymentType, (v) => setDialogState(() => paymentType = v)),
                  const SizedBox(width: 8),
              ...
              // ── Trade-In Fields ──
              if (txnType == 'Trade-In') ...[
                _editField("Car Name (e.g. Kia Sportage 2022)", tradeCarNameCtrl),
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Icon(FluentIcons.info, size: 14, color: AppTheme.success),
                    const SizedBox(width: 8),
                    Expanded(child: Text("This amount will be CREDITED to ${c['name']}'s account. The traded car will be added back to inventory.", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, color: AppTheme.success))),
                  ]),
                ),
                const SizedBox(height: 14),
              ...
              // Salesman
              Text("Salesman", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, fontWeight: FontWeight.w600, color: AppTheme.textSecondary)),
              const SizedBox(height: 8),
              ComboBox<String>(
                value: selectedSalesman,
                placeholder: const Text("Select Salesman...", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 13)),
                isExpanded: true,
                items: _salesmen.map((s) => ComboBoxItem<String>(value: s, child: Text(s, style: const TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 13)))).toList(),
                onChanged: (v) => setDialogState(() => selectedSalesman = v),
              ),
              const SizedBox(height: 14),
            ...
            FilledButton(
              ...
              onPressed: () {
                ...
                if (txnType == 'Sell Car') {
                  ...
                  final price = int.tryParse(amountCtrl.text) ?? selectedCarPrice;
                  setState(() {
                  ...
                  if (tradeCarNameCtrl.text.trim().isEmpty) return;
                  final amt = int.tryParse(tradeAmountCtrl.text) ?? 0;
                  if (amt <= 0) return;
                  setState(() {
                    (c['ledger'] as List<Map<String, dynamic>>).add({
  ...
  void _showEditTransactionDialog(Map<String, dynamic> customer, int entryIndex) {
    ...
    if (selectedSalesman != null && selectedSalesman.isEmpty) selectedSalesman = null;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDialogState) {
        return ContentDialog(
          title: const Text("Edit Transaction", style: TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700)),
          constraints: const BoxConstraints(maxWidth: 560),
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              // Show type as read-only badge
              Row(children: [
                Text("Type: ", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, fontWeight: FontWeight.w600, color: AppTheme.textSecondary)),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
                  decoration: BoxDecoration(color: _txnColor(originalType).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(6)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(_txnIcon(originalType), size: 14, color: _txnColor(originalType)),
                    const SizedBox(width: 6),
                    Text(originalType, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, fontWeight: FontWeight.w700, color: _txnColor(originalType))),
                  ]),
                ),
              ]),
  ...
  void _showDeleteLedgerEntryDialog(Map<String, dynamic> customer, int entryIndex) {
    final entry = (customer['ledger'] as List<Map<String, dynamic>>)[entryIndex];
    showDialog(
      context: context,
      builder: (ctx) => ContentDialog(
        title: const Text("Delete Entry", style: TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700)),
        content: Text(
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () {
              setState(() {
                final removed = (customer['ledger'] as List<Map<String, dynamic>>).removeAt(entryIndex);
                // If a Car Sale was deleted, add the car back to inventory
                if (removed['type'] == 'Car Sale') {
                  final price = removed['debit'] as int;
                  _availableCars.add({'name': removed['details'], 'price': price, 'regNo': 'TBD'});
                }
                // If a Trade-In was deleted, remove the car from inventory
                if (removed['type'] == 'Trade-In') {
                  final carName = (removed['details'] as String).replaceAll(' (Sold Back)', '');
  ...
  // ═══════════════════════════════════════════════════
  Future<void> _showTransactionReceipt(Map<String, dynamic> customer, int entryIndex) async {
    final ledger = customer['ledger'] as List<Map<String, dynamic>>;
    final entry = ledger[entryIndex];
      ...
      build: (pw.Context context) {
        return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          ...
          pw.Center(child: pw.Column(children: [
            pw.Text("INAM MOTORS", style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#6C5DD3'))),
            pw.SizedBox(height: 2),
            pw.Text("Car Showroom & Dealership", style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey600)),
            pw.SizedBox(height: 6),
            pw.Container(height: 2, width: 160, color: PdfColor.fromHex('#6C5DD3')),
          ])),
          pw.SizedBox(height: 16),

          pw.Center(child: pw.Container(
            padding: const pw.EdgeInsets.symmetric(horizontal: 16, vertical: 6),
            decoration: pw.BoxDecoration(color: PdfColor.fromHex('#F0EEFF'), borderRadius: pw.BorderRadius.circular(4)),
            child: pw.Text("TRANSACTION RECEIPT", style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColor.fromHex('#6C5DD3'))),
          )),
          pw.SizedBox(height: 16),

          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
            pw.Text("Date: ${entry['date']}", style: const pw.TextStyle(fontSize: 10)),
            pw.Text("Receipt #: RCP-${(customer['name'] as String).split(' ').first.toUpperCase()}-${entryIndex + 1}", style: const pw.TextStyle(fontSize: 10)),
          ]),
          pw.Divider(),
          ...
          _pdfRow("Amount", '${formatFullPrice(amount)} (${debit > 0 ? "Debit" : "Credit"})'),
          if (entry['salesman'] != null && (entry['salesman'] as String).isNotEmpty)
            _pdfRow("Salesman", entry['salesman']),
          pw.SizedBox(height: 12),

          // Documentation Status (for Car Sale & Trade-In)
          if (type == 'Car Sale' || type == 'Trade-In') ...[
            pw.Text("DOCUMENTATION STATUS", style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: PdfColors.grey700)),
            pw.SizedBox(height: 6),
            pw.Row(children: [
              pw.Expanded(child: _pdfDocItem("File", entry['file'] == true)),
              pw.Expanded(child: _pdfDocItem("Smart Card", entry['smartCard'] == true)),
            ]),
          ...
          pw.Container(
            ...
            child: pw.Column(children: [
              pw.Text("BALANCE SUMMARY", style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 8),
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text("Previous Balance:", style: const pw.TextStyle(fontSize: 10)),
                pw.Text(
                  prevBalance == 0 ? 'Settled' : '${formatFullPrice(prevBalance.abs())} ${prevBalance > 0 ? 'DR' : 'CR'}',
                  style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: prevBalance > 0 ? PdfColors.red : prevBalance < 0 ? PdfColors.green : PdfColors.grey700),
                ),
              ]),
              pw.SizedBox(height: 4),
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text("This Transaction:", style: const pw.TextStyle(fontSize: 10)),
                pw.Text(
                  '${debit > 0 ? '+' : '-'} ${formatFullPrice(amount)}',
                  style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: debit > 0 ? PdfColors.red : PdfColors.green),
                ),
              ]),
              pw.Divider(),
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text("New Balance:", style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold)),
                pw.Text(
                  newBalance == 0 ? 'Settled' : '${formatFullPrice(newBalance.abs())} ${newBalance > 0 ? 'DR (Outstanding)' : 'CR (Credit)'}',
                  style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: newBalance > 0 ? PdfColors.red : newBalance < 0 ? PdfColors.green : PdfColors.grey700),
                ),
              ]),
            ]),
          ),
          ...
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
            pw.Column(children: [
              ...
              pw.SizedBox(height: 4),
              pw.Text("Customer Signature", style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey500)),
            ]),
            pw.Column(children: [
              pw.Container(width: 100, height: 0.5, color: PdfColors.grey400),
              pw.SizedBox(height: 4),
              pw.Text("Authorized Signature", style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey500)),
            ]),
          ]),
          pw.SizedBox(height: 8),
          pw.Center(child: pw.Text("Generated by Inam Motors System", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey400))),
        ]);
      },
    ));

    final bytes = await pdf.save();
    await Printing.sharePdf(bytes: bytes, filename: 'Receipt_${customer['name'].toString().replaceAll(' ', '_')}_${entryIndex + 1}.pdf');
  }
  ...
  Future<void> _showLedgerPdf(Map<String, dynamic> c) async {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (pw.Context context) {
        ...
        return [
          ...
          pw.Text("Statement of Account", style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.Divider(),
          pw.SizedBox(height: 8),
          ...
          // Summary
          pw.Row(children: [
            _pdfStatBox("Total Charged", formatFullPrice(totalDebit), PdfColor.fromHex('#EF4444')),
            pw.SizedBox(width: 12),
            _pdfStatBox("Total Paid", formatFullPrice(totalCredit), PdfColors.green),
            pw.SizedBox(width: 12),
            _pdfStatBox(
              balance > 0 ? "Outstanding" : balance < 0 ? "Credit Balance" : "Balance",
          ...
          // Ledger Table
          pw.Text("Ledger Details", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 8),
          if (ledger.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              headers: ['Date', 'Type', 'Details', 'Debit', 'Credit', 'Balance', 'Salesman'],
              data: ledger.map((e) {
                ...
                return [
                  ...
                  e['type'],
                  details,
                  (e['debit'] as int) > 0 ? formatFullPrice(e['debit'] as int) : '-',
                  (e['credit'] as int) > 0 ? formatFullPrice(e['credit'] as int) : '-',
                  '${formatFullPrice(runBal.abs())}${runBal > 0 ? ' DR' : runBal < 0 ? ' CR' : ''}',
                  e['salesman'] ?? '',
                ];
              }).toList(),
            ),
          ...
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
            pw.Column(children: [
              ...
              pw.SizedBox(height: 4),
              pw.Text("Customer Signature", style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey500)),
            ]),
            pw.Column(children: [
              pw.Container(width: 120, height: 0.5, color: PdfColors.grey400),
              pw.SizedBox(height: 4),
    ...
    final bytes = await pdf.save();
    await Printing.sharePdf(bytes: bytes, filename: 'Ledger_${c['name'].toString().replaceAll(' ', '_')}.pdf');
  }
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: value ? AppTheme.success.withValues(alpha: 0.5) : AppTheme.divider),
        ),
        child: Row(children: [
          Icon(value ? FluentIcons.checkbox_composite : FluentIcons.checkbox, size: 16, color: value ? AppTheme.success : AppTheme.textMuted),
          const SizedBox(width: 8),
          Icon(icon, size: 13, color: value ? AppTheme.success : AppTheme.textMuted),
          const SizedBox(width: 6),
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, size: 18, color: sel ? Colors.white : AppTheme.textSecondary),
            const SizedBox(height: 4),
            Text(label, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, fontWeight: FontWeight.w600, color: sel ? Colors.white : AppTheme.textPrimary)),
          ]),
        ),
      ),
    );
        ...
        children: [
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 4),
              Text("Customer ledger system \u2014 track sales, payments & trade-ins", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: isNarrow ? 12 : 14, color: AppTheme.textSecondary)),
            ])),
            FilledButton(
              style: ButtonStyle(
          ...
          if (isNarrow)
            Column(children: [
              ...
              Row(children: [
                StatCard(label: "With Balance", value: "$_withBalanceCount", icon: FluentIcons.alert_solid, color: AppTheme.error),
                const SizedBox(width: 12),
            ...
            Row(children: [
              StatCard(label: "Total Customers", value: "${_customers.length}", icon: FluentIcons.people, color: AppTheme.primary),
              const SizedBox(width: 16),
              StatCard(label: "VIP Customers", value: "$_vipCount", icon: FluentIcons.diamond_user, color: AppTheme.warning),
              const SizedBox(width: 16),
              StatCard(label: "With Balance Due", value: "$_withBalanceCount", icon: FluentIcons.alert_solid, color: AppTheme.error),
              const SizedBox(width: 16),
              StatCard(label: "Leads", value: "$_leadCount", icon: FluentIcons.people_add, color: AppTheme.success),
            ]),
      ...
      child: Row(children: [
        Expanded(flex: 2, child: Row(children: [
          ...
          Flexible(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(c['city'] ?? '', overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 10, color: AppTheme.textMuted)),
          ])),
        ])),
        ...
        SizedBox(width: 140, child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          if (balance > 0)
            Text("Owes", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 9, color: AppTheme.error.withValues(alpha: 0.7))),
          if (balance < 0)
            Text("Credit", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 9, color: AppTheme.success.withValues(alpha: 0.7))),
        ])),
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: MouseRegion(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Text("View Statement", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white)),
                  SizedBox(width: 6),
    ...
    return MouseRegion(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Text("$carCount cars", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, color: AppTheme.textMuted)),
          const Spacer(),
          Container(
        ...
        children: [
          ...
          Row(children: [
            ...
            Container(
              ...
              child: Center(child: Text(c['name'].toString().substring(0, 1), style: const TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700, fontSize: 18, color: AppTheme.primary))),
            ),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Flexible(child: Text(c['name'], style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: isNarrow ? 18 : 24, fontWeight: FontWeight.w700, color: AppTheme.textPrimary))),
                const SizedBox(width: 10),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
                  decoration: BoxDecoration(color: typeColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(5)),
              ...
              const SizedBox(height: 2),
              Text("${c['phone']}  \u2022  ${c['cnic']}  \u2022  ${c['city']}", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, color: AppTheme.textMuted)),
            ])),
            // Action buttons
            if (!isNarrow) ...[
              FilledButton(
                style: ButtonStyle(backgroundColor: WidgetStateProperty.all(AppTheme.primary), shape: WidgetStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), padding: WidgetStateProperty.all(const EdgeInsets.symmetric(horizontal: 16, vertical: 10))),
          ...
          if (isNarrow) ...[
            ...
            Row(children: [
              Expanded(
                child: FilledButton(
                  style: ButtonStyle(backgroundColor: WidgetStateProperty.all(AppTheme.primary), shape: WidgetStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), padding: WidgetStateProperty.all(const EdgeInsets.symmetric(vertical: 10))),
                  onPressed: () => _showAddTransactionDialog(c),
          ...
          if (isNarrow)
            Column(children: [
              ...
              const SizedBox(height: 12),
              Row(children: [
                StatCard(label: "Balance", value: _balanceText(balance), icon: FluentIcons.calculator, color: _balanceColor(balance)),
                const SizedBox(width: 12),
                StatCard(label: "Cars Bought", value: "$carCount", icon: FluentIcons.car, color: AppTheme.primary),
              ]),
            ])
          else
            Row(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: _balanceColor(balance).withValues(alpha: 0.2)),
            ),
            child: Row(children: [
              Icon(
                balance > 0 ? FluentIcons.warning : balance < 0 ? FluentIcons.completed : FluentIcons.accept_medium,
                size: 20,
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(
                  balance > 0 ? "Outstanding Balance" : balance < 0 ? "Credit Balance (We Owe Customer)" : "Account Settled",
                  style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 14, fontWeight: FontWeight.w700, color: _balanceColor(balance)),
                ),
                const SizedBox(height: 2),
                Text(
                  balance > 0
          ...
          // ── LEDGER TABLE ──
          Row(children: [
            Text("Statement of Account", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 16, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
            const Spacer(),
            Text("${ledger.length} transactions", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, color: AppTheme.textMuted)),
          ]),
          const SizedBox(height: 12),
            ...
            Container(
              ...
              child: Column(children: [
                Container(
                  ...
                  child: const Icon(FluentIcons.document_set, size: 28, color: AppTheme.primary),
                ),
                const SizedBox(height: 12),
                Text("No transactions yet", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 15, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
                const SizedBox(height: 4),
                Text("Click 'Add Transaction' to sell a car, receive payment, or record a trade-in", style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, color: AppTheme.textMuted)),
              ]),
            )
          else ...[
            // Table Header
            if (!isNarrow) Container(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
              decoration: BoxDecoration(color: AppTheme.background, borderRadius: const BorderRadius.only(topLeft: Radius.circular(8), topRight: Radius.circular(8))),
              child: Row(children: [
  ...
  List<Widget> _buildLedgerRows(Map<String, dynamic> customer, List<Map<String, dynamic>> ledger, bool isNarrow) {
    ...
    for (int i = 0; i < ledger.length; i++) {
      ...
      if (isNarrow) {
        rows.add(_buildLedgerCard(customer, i, e, debit, credit, runningBalance));
      } else {
        ...
        rows.add(Container(
          ...
          child: Row(children: [
            SizedBox(width: 90, child: Text(e['date'], style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, color: AppTheme.textSecondary))),
            SizedBox(width: 80, child: Row(children: [
              Container(
                padding: const EdgeInsets.all(4),
                decoration: BoxDecoration(color: _txnColor(e['type']).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(5)),
                child: Icon(_txnIcon(e['type']), size: 12, color: _txnColor(e['type'])),
              ),
              const SizedBox(width: 6),
              Flexible(child: Text(e['type'], overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 11, fontWeight: FontWeight.w600, color: _txnColor(e['type'])))),
            ])),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(e['details'], overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, color: AppTheme.textPrimary)),
              Row(children: [
                if (salesman.isNotEmpty) ...[
                  Icon(FluentIcons.people, size: 10, color: AppTheme.textMuted),
                  const SizedBox(width: 3),
                  Text(salesman, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 10, color: AppTheme.textMuted)),
                ],
                if (salesman.isNotEmpty && hasDocs) const SizedBox(width: 8),
                if (hasDocs) ...[
  ...
  Widget _miniDocBadge(String label, bool ok) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: ok ? AppTheme.success.withValues(alpha: 0.12) : AppTheme.error.withValues(alpha: 0.08),
        borderRadius: BorderRadius.circular(3),
      ),
      child: Text(label, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 8, fontWeight: FontWeight.w700, color: ok ? AppTheme.success : AppTheme.error.withValues(alpha: 0.5))),
    );
  }

  Widget _buildLedgerCard(Map<String, dynamic> customer, int index, Map<String, dynamic> e, int debit, int credit, int runningBalance) {
    final hasDocs = e['type'] == 'Car Sale' || e['type'] == 'Trade-In';
    final salesman = e['salesman'] as String? ?? '';
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: _txnColor(e['type']).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(6)),
            child: Icon(_txnIcon(e['type']), size: 14, color: _txnColor(e['type'])),
          ),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(e['type'], style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 12, fontWeight: FontWeight.w700, color: _txnColor(e['type']))),
        ...
        if (salesman.isNotEmpty || hasDocs) ...[
          ...
          Wrap(spacing: 8, runSpacing: 4, children: [
            if (salesman.isNotEmpty) Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(FluentIcons.people, size: 10, color: AppTheme.textMuted),
              const SizedBox(width: 3),
              Text(salesman, style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 10, color: AppTheme.textMuted)),
            ]),
            if (hasDocs) Row(mainAxisSize: MainAxisSize.min, children: [
              _miniDocBadge("F", e['file'] == true),
              const SizedBox(width: 3),
              _miniDocBadge("SC", e['smartCard'] == true),
              const SizedBox(width: 3),
